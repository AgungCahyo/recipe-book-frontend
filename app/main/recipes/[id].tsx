...
import { useIngredientsData, Ingredient } from 'context/ingredients/IngredientsProvider';
import { useNavigationState } from 'context/NavigationContext';
import ConfirmModal from 'app/components/ConfirmModal';

const screenWidth = Dimensions.get('window').width;

// OPTIMASI 1: Memoized components
...
export default function RecipeDetailPage() {
  ...
  const ingredients = useIngredientsData(); // OPTIMASI 3: Use specific selector
  ...
  // OPTIMASI 4: Get recipe dengan early return untuk invalid ID
  ...
  // OPTIMASI 5: Stable ingredients mapping
  ...
  // OPTIMASI 6: Memoized updated recipe calculation
  const updatedRecipe = useMemo(() => {
    if (!originalRecipe || !ingredientsMap.size) return null;
  ...
  // OPTIMASI 8: Memoized selling price hook
  ...
  // OPTIMASI 11: Memoized edit handler
  const handleEdit = useCallback(() => {
    if (!updatedRecipe) return;

    InteractionManager.runAfterInteractions(() => {
      router.push({
        pathname: '/main/recipes/recipeForm',
        params: { id: updatedRecipe.id }
      });
    });
  }, [updatedRecipe, router]);

  // OPTIMASI 12: Memoized slider marker
  ...
  // OPTIMASI 13: Memoized slider styles
  const sliderStyles = useMemo(() => ({
    selected: { backgroundColor: '#204c4b', height: 6, borderRadius: 999 },
    unselected: { backgroundColor: isDark ? '#334155' : '#E5E7EB', height: 6, borderRadius: 999 },
    ...
    return (
      <View style={[styles.loadingContainer, { backgroundColor: isDark ? '#000' : '#fff' }]}>
        <Text style={{ color: isDark ? '#fff' : '#000' }}>
          {!id ? 'ID resep tidak valid' : 'Resep tidak ditemukan'}
        </Text>
      </View>
    );
  }
  ...
  // Saat harga manual sudah disimpan, otomatis keluar dari mode editing
  useEffect(() => {
    if (isManual) {
  ...
  // Handler tombol edit harga jual
...
const handleSaveAndClose = () => {
  ...
  return (
    <View className="flex-1 bg-accent/60 dark:bg-black">
      ...
      >
        {/* Gambar Resep - Optimized rendering */}
        {Array.isArray(updatedRecipe.imageUris) && updatedRecipe.imageUris.length > 0 && (
          ...
          >
            {updatedRecipe.imageUris.map((uri, index) => (
              <MemoizedImage key={`image-${index}`} uri={uri} index={index} />
            ))}
          </ScrollView>
        )}
        ...
        <View className="px-5 pb-28">
          {/* Bahan-bahan */}
          <Text className="text-xl font-semibold text-dark dark:text-zinc-200 mb-1">
            Bahan-bahan
          </Text>
          <View className="space-y-2 mb-2">
            {(showAllIngredients ? updatedRecipe.ingredients : updatedRecipe.ingredients.slice(0, MAX_VISIBLE)).map((item) => (
              <MemoizedIngredientItem key={`ingredient-${item.id}`} item={item} />
            ))}
          </View>
          {updatedRecipe.ingredients.length > MAX_VISIBLE && (
            <TouchableOpacity onPress={() => setShowAllIngredients(!showAllIngredients)}>
              <Text className="text-primary text-left font-semibold">
                {showAllIngredients ? 'Tutup' : `Lihat Selengkapnya`}
              </Text>
            </TouchableOpacity>
          )}

          {/* Langkah-langkah */}
          <Text className="text-xl font-semibold text-dark dark:text-zinc-200 mb-1 mt-6">
            Langkah-langkah
          </Text>
          {updatedRecipe.description ? (
            <>
              {showAllSteps
                ? <Text className="text-lg text-zinc-700 dark:text-zinc-300 mb-1 leading-relaxed">{updatedRecipe.description}</Text>
                : <Text className="text-lg text-zinc-700 dark:text-zinc-300 mb-1 leading-relaxed" numberOfLines={5}>{updatedRecipe.description}</Text>
              }
              {updatedRecipe.description.split('\n').length > MAX_VISIBLE && (
                <TouchableOpacity onPress={() => setShowAllSteps(!showAllSteps)}>
                  <Text className="text-primary text-center font-semibold">
                    {showAllSteps ? 'Tutup' : 'Lihat Selengkapnya'}
                  </Text>
                </TouchableOpacity>
              )}
            </>
          ) : (
            <Text className="text-primary/50 mb-4 italic text-lg">
              Tidak ada deskripsi langkah.
            </Text>
          )}
          ...
          <View className="flex-row justify-between items-center mb-1">
            ...
            <Text className="text-xs text-zinc-500 dark:text-zinc-400">
              HPP: Rp {totalHPP.toLocaleString('id-ID')}
            </Text>
          </View>
         ...
         <View className="px-5">
          {!isManual || isEditingPrice ? (
            // Tampilkan input harga jual saat belum manual atau sedang edit
            <>
              <Text className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mt-6 mb-2">
                Tetapkan Harga Jual Manual
              </Text>
              <View className="gap-3 justify-center flex-row">
                ...
                {isManual && (
                  <TouchableOpacity
                    ...
                    className="py-3 px-4 ml-2 rounded-xl border border-primary dark:border-zinc-600"
                  >
                    <Text className="text-center text-primary dark:text-gray-300 text-sm font-semibold">
                      Cancel
                    </Text>
                  </TouchableOpacity>
                )}
            ...
            // Tampilkan harga jual final + tombol edit jika sudah manual dan gak sedang edit
            <View className="mt-6 items-center">
              ...
              <TouchableOpacity
                ...
                className="mt-3 py-2 px-6 rounded-2xl bg-primary dark:bg-blue-500 active:opacity-85"
                activeOpacity={0.7}
              >
                <Text className="text-white font-semibold">Edit Harga Jual</Text>
              </TouchableOpacity>
              <Text className="text-xs text-zinc-400 mt-1 italic">
                Ditentukan manual oleh pengguna
              </Text>
              <Text className="text-xs text-zinc-400 mt-1 italic">
      ...
      <View className="absolute bottom-0 left-0 right-0 px-5 py-4 bg-accent dark:bg-black border-t border-primary/50 mx-2 dark:border-zinc-800 flex-row gap-3">
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text className="text-center text-zinc-900 dark:text-white text-sm font-semibold">
            Hapus
          </Text>
        </TouchableOpacity>
      </View>
