    ...
    <ScrollView className="flex-1 bg-background-light dark:bg-background-dark">
      <View className="p-6">
        ...
        <View className="items-center mb-8">
          <View className="relative mt-4 mb-4">
            {user?.photoURL ? (
              ...
              <View className="w-28 h-28 rounded-full bg-accent dark:bg-accent-dark items-center justify-center shadow-lg">
                <Text className="text-3xl font-bold text-dark dark:text-dark-dark">
          ...
          <Text className="text-2xl font-bold text-dark dark:text-dark-dark mb-1">
            {storedProfile?.name || 'No Name'}
          </Text>
          <Text className="text-sm text-dark/50 dark:text-zinc-400">
            Member sejak{' '}
        ...
        <View className=" mb-8">
          ...
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md mb-5">
            <Text className="text-sm text-dark dark:text-zinc-400 mb-1">Nama Lengkap</Text>
            {isEditing ? (
              <TextInput
                value={editName}
                onChangeText={setEditName}
                className="text-base text-dark dark:text-dark-dark bg-background-light dark:bg-dark px-3 py-2 rounded-lg border border-primary dark:border-zinc-600"
                placeholder="Masukkan nama"
              />
            ) : (
              <Text className="text-base text-dark dark:text-dark-dark">{storedProfile?.name || '-'}</Text>
            )}
          </View>

          {/** Usia */}
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md mb-5">
            <Text className="text-sm text-dark dark:text-zinc-400 mb-1">Usia</Text>
            {isEditing ? (
              <TextInput
                value={editAge}
                onChangeText={setEditAge}
                keyboardType="numeric"
                className="text-base text-dark dark:text-dark-dark bg-background-light px-3 py-2 rounded-lg border border-primary dark:border-zinc-600"
                placeholder="Masukkan usia"
              />
            ) : (
              <Text className="text-base text-dark dark:text-dark-dark">
          ...
          {/** Email */}
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md mb-5">
            <Text className="text-base text-dark dark:text-zinc-400 mb-1">Email</Text>
            <Text className="text-base text-dark/50 dark:text-dark-dark">{user?.email || '-'}</Text>
          ...
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md">
            <Text className="text-sm text-dark dark:text-zinc-400 mb-1">User ID</Text>
            <Text className="text-xs text-dark/50 dark:text-zinc-300 font-mono">{user?.uid || '-'}</Text>
          </View>
        </View>
        ...
        <View className="space-y-3">
          {isEditing ? (
            <View className="flex-row space-x-3 gap-4">
              ...
              <TouchableOpacity
                ...
                className="flex-1 bg-dark dark:bg-gray-600 py-3 rounded-2xl mb-5 shadow-lg"
              >
          ...
          <TouchableOpacity
            ...
            className="bg-dark py-3 rounded-2xl shadow-lg"
          >
        ...
        {/* Footer */}
        <View className="mt-8 pt-6 border-t border-dark/50 dark:border-zinc-700">
          <Text className="text-xs text-center text-dark/50 dark:text-zinc-500">
